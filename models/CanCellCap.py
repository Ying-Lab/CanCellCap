...
class common(nn.Module):
    # 无mask时使用 ad weight=0  gene_drop_weight=0
    # 无common时  self.args.weight_ad_train=0
    # 无moe self.args.weight_tissue =0
    def __init__(self, args):
        ...
        self.mask_decoder = mask_decoder(args, self.common_featurizer.out_size)
        self.classifier = feat_classifier(args.num_classes, self.featurizer.out_size)
    ...
    def update(self, minibatches, opt, sch):
        ...
        loss = cancer_mean +  ad_mean #+ penalty_weight * penalty
        ...
        return {'loss': loss.item(),'cancer_mean': cancer_mean.item(), 'ad_mean':ad_mean.item()}
    ...
    def predict(self, x):
        ...
        return logit, 0, logit_common_domain, x_reconstruct


class common_moe(nn.Module):
    def __init__(self, args):
        """
        The network contains a feature extractor and a classifier
        Args:
            args (object) : args object generated by opt_util.py
        """
        super(common_moe, self).__init__()
        self.featurizer = get_fea(args)
        self.model_name = 'common_moe'
        self.moe = MoE(args.num_tissue, len(args.HVG_list), self.featurizer.out_size)
        self.tissue_classifier = feat_classifier(args.num_tissue, self.featurizer.out_size)
        self.ad_classifier = ad_classifier_class(args.num_tissue, self.featurizer.out_size)
        self.common_featurizer = get_fea(args)
    ...
    def update(self, minibatches, opt, sch):
        ...
        loss = cancer_mean + tissue_mean +  ad_mean 
        ...
        return {'loss': loss.item(),'cancer_mean': cancer_mean.item(), 'tissue_mean':tissue_mean.item(), 'ad_mean':ad_mean.item()}
    ...
    def predict(self, x):
        ...
        return logit, moe_wight, logit_common_domain, 0

class moe_mask(nn.Module):
    def __init__(self, args):
        """
        The network contains a feature extractor and a classifier
        Args:
            args (object) : args object generated by opt_util.py
        """
        super(moe_mask, self).__init__()
        self.featurizer = get_fea(args)
        self.model_name = 'moe_mask'
        self.moe = MoE(args.num_tissue, len(args.HVG_list), self.featurizer.out_size)
        self.tissue_classifier = feat_classifier(args.num_tissue, self.featurizer.out_size)
        self.ad_classifier = ad_classifier_class(args.num_tissue, self.featurizer.out_size)
        self.common_featurizer = get_fea(args)
    ...
    def update(self, minibatches, opt, sch):
        ...
        # penalty = ((cancer_losses - cancer_mean) ** 2).mean()
        loss = cancer_mean + tissue_mean + reconstruct_mean#+ penalty_weight * penalty
        ...
        return {'loss': loss.item(),'cancer_mean': cancer_mean.item(), 'tissue_mean':tissue_mean.item(), 'reconstruct_mean':reconstruct_mean.item()}
    ...
    def predict(self, x):
        ...
        x_reconstruct = self.mask_decoder(x_cat)
        return logit, moe_wight, 0, x_reconstruct


class common_mask(nn.Module):
    ...
    def __init__(self, args):
        ...
        Args:
            args (object) : args object generated by opt_util.py
        """
        super(common_mask, self).__init__()
        self.featurizer = get_fea(args)
        self.model_name = 'common_mask'
        self.moe = MoE(args.num_tissue, len(args.HVG_list), self.featurizer.out_size)
        self.tissue_classifier = feat_classifier(args.num_tissue, self.featurizer.out_size)
        self.ad_classifier = ad_classifier_class(args.num_tissue, self.featurizer.out_size)
        self.common_featurizer = get_fea(args)
    ...
    def update(self, minibatches, opt, sch):
        ...
        # penalty = ((cancer_losses - cancer_mean) ** 2).mean()
        loss = cancer_mean +  ad_mean + reconstruct_mean#+ penalty_weight * penalty
        ...
        return {'loss': loss.item(),'cancer_mean': cancer_mean.item(), 'ad_mean':ad_mean.item(), 'reconstruct_mean':reconstruct_mean.item()}
