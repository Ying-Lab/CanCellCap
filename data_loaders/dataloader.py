...
def test_dataloader_gene_cell(args, val_dataset, shuffle_state=True):
    """
    Create validating dataloaders
    ...
    # print(f"loading dataset: {SPL_FILE}")
    raw_df = pd.merge(gene_list,raw_df,how='left',left_index=True,right_index=True).fillna(0)
    raw_df = raw_df[~raw_df.index.duplicated()]
    raw_df = raw_df.loc[gene_list.index]
    raw_df = raw_df.T
...
def test_dataloader_cell_gene(args, val_dataset, shuffle_state=False):
    SPL_FILE = val_dataset
    batch_size = args.batch_size
    ...
    # raw_df.set_index('Row.names', inplace=True)
    # print(f"loading dataset: {SPL_FILE}")
...
def val_domian_loader_id(args, val_dataset, shuffle_state=False):
    SPL_FILE = val_dataset
    ...
    return external_val_loader_dict, raw_df.columns.tolist()
